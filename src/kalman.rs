...
//! the STATE only; it is never fed into the forward prediction, so it cannot double-count with
//! `heating.bias_correction` (which corrects the forward flux, a different object).
//!
...
impl KalmanFilter {
    ...
    /// Zones this filter was built for (the measured set at startup).
    pub fn zones(&self) -> impl Iterator<Item = &str> {
        self.gains.iter().map(|(z, _, _)| z.as_str())
    }
